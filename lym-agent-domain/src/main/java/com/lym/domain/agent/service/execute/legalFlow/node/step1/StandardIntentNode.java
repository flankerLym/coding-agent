...
/**
 * 标准意图识别节点：
 * 兼容原有实现，但从 MetaIntentRouterNode 进入。
 */
@Slf4j
@Service
public class StandardIntentNode extends AbstractLegalLlmNodeSupport {
    ...
    public String apply(ExecuteCommandEntity request,
        ...
        context.addTrace("[Intent] intent=" + context.getIntentCode()
        ...
        LegalFlowSseUtils.sendAnalysis(
                context.getEmitter(),
                4,
                "StandardIntentNode(openAiChatClient)：intent=" + context.getIntentCode()
                        + " confidence=" + context.getIntentConfidence()
                        + " reason=" + context.getIntentReason(),
                context.getSessionId());

        log.info("标准意图识别完成，clientId:{} beanName:{} result:{}",
